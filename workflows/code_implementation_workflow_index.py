...
"""
    ...
    async def _pure_code_implementation_loop_openai_responses(
        ...
        while iteration < max_iterations:
            ...
            # Only send instructions on the first call. Continuations should rely on
            # `previous_response_id` state to avoid resending the system prompt.
        ...
        async def init_openai():
            ...
            try:
                ...
                if base_url_raw:
                    ...
                    # `base_url` is expected to already be normalized by load_api_config,
                    # but fall back to the derived SDK base URL if needed.
                    if not base_url:
                ...
                try:
                    if endpoint_hint == OpenAIEndpointHint.RESPONSES:
                        # Fail-fast: verify both Responses API *and* continuation via
                        # `previous_response_id` are supported by the configured gateway.
                        self.openai_responses_mode = "stateful"
